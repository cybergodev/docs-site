...
description: "Полное руководство по настройке пула соединений и прокси HTTPC. Подробное описание настройки параметров пула соединений ConnectionConfig, конфигурация прямого HTTP-прокси и SOCKS5-прокси, пользовательский резолвер DNS-over-HTTPS, диагностика утечки соединений и стратегия управления неактивными соединениями через IdleConnTimeout"
---
...
cfg.Connection.MaxIdleConns = 100         // Максимальное количество неактивных соединений глобально
cfg.Connection.MaxConnsPerHost = 20       // Максимальное количество соединений на хост
cfg.Timeouts.IdleConn = 120 * time.Second // Время удержания неактивных соединений
```
...
|------|------|------|
| `MaxIdleConns` | 50 | Максимальное количество неактивных соединений глобально |
| `MaxConnsPerHost` | 10 | Максимальное количество соединений на хост (активных + неактивных) |
| `IdleConn` | 90s | Таймаут неактивного соединения, закрывается по истечении |
| `Dial` | 10s | Таймаут установки соединения |
| `TLSHandshake` | 10s | Таймаут рукопожатия TLS |
| `ResponseHeader` | 30s | Таймаут ожидания заголовков ответа |
...
| Стандартный сервис | 50 | 10 | 90s |
| Редкие запросы | 10 | 2 | 30s |
| Внутренние микросервисы | 50 | 10 | 60s |

:::tip Подсказка
`MaxConnsPerHost` включает активные и неактивные соединения. Новые запросы, превышающие этот лимит, будут ожидать освобождения соединения в очереди.
:::
...
|------|------|----------|
| Большое количество TIME_WAIT | Слишком короткий таймаут неактивных соединений | Увеличить таймаут `IdleConn` |
...
- [Оптимизация производительности](./performance) - руководство по оптимизации производительности
- [Конфигурация API](../api-reference/config) - справочник по конфигурации соединений
