...
:::warning
Допускаются только абсолютные URL с протоколами `http://` и `https://`, остальные протоколы отклоняются (защита от SSRF).
:::
...
- [Интерфейсы](./interfaces) - справочник по интерфейсам Client, Doer
