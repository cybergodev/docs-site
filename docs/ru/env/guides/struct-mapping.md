---
title: Отображение в структуру - CyberGo env | Переменные окружения в структуру
description: CyberGo env библиотека полное руководство по отображению в структуру, автоматическое отображение переменных окружения в поля структуры Go через теги env структуры, поддержка вложенных структур, указателей и срезов, пользовательское преобразование типов, значения по умолчанию, валидация обязательных полей.
---

# Отображение в структуру

Используйте теги структуры для автоматического отображения переменных окружения в структуры Go, обеспечивая типобезопасное управление конфигурацией.
...
```go
type Config struct {
    Host string `env:"SERVER_HOST"`  // Отображает SERVER_HOST
    Port int64  `env:"PORT"`         // Отображает PORT
}
```
...
type Config struct {
    ...
    // Использовать вертикальную черту как разделитель
...
- Пробелы вокруг разделителя автоматически удаляются
...
if err := env.ParseInto(&cfg); err != nil {
    ...
    if errors.As(err, &parseErr) {
        log.Fatalf("Ошибка парсинга в %s:%d", parseErr.File, parseErr.Line)
    }
    log.Fatal(err)
}
```
...
cfg := Config{}
if err := env.ParseInto(&cfg); err != nil {
    // Ошибка преобразования типа вернёт ошибку
}
```
...
- [Пакетные функции - ParseInto](/ru/env/api-reference/functions#parseinto) - Справка по функции ParseInto
