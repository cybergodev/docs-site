---
title: Контрольный список для продакшена - CyberGo env | Проверка безопасности
description: Полный контрольный список безопасности перед развёртыванием в продакшен библиотеки CyberGo env, охватывающий управление правами доступа к файлам конфигурации, настройку правил валидации ключей и значений, подтверждение включения аудиторского протоколирования, методы безопасной обработки конфиденциальных данных, стратегии обработки ошибок и настройку параметров производительности и безопасности.
---

# Контрольный список для продакшена

Контрольный список перед развёртыванием приложения в продакшен.

::: tip Совет Концепция безопасности
Архитектура безопасности и основные функции подробно описаны в [Обзоре безопасности](/ru/env/security/).
:::

## Проверки перед развёртыванием

### Безопасность файлов

- [ ] Файл `.env.production` существует
- [ ] Права доступа файла `600` или строже
- [ ] Конфиденциальные файлы добавлены в `.gitignore`
- [ ] Файл конфигурации не содержит плейсхолдеров (например, `change-me`, `xxx`)

```bash
# Проверка прав доступа
ls -la .env.production
# Должно отображаться: -rw------- (600)

# Исправление прав доступа
chmod 600 .env.production
```

### Валидация конфигурации

- [ ] Все обязательные ключи установлены
- [ ] Конфиденциальные значения не пусты
- [ ] Формат значений корректен (URL, порты и т.д.)
- [ ] Нет жёстко закодированных ключей

```go
cfg := env.ProductionConfig()
cfg.RequiredKeys = []string{
    "DB_HOST", "DB_PORT", "DB_USER", "DB_PASSWORD",
    "API_KEY", "API_URL",
}
cfg.FailOnMissingFile = true
```

## Проверка конфигурации безопасности

### Аудиторский журнал

- [ ] Аудиторский журнал включён
- [ ] Каталог логов доступен для записи
- [ ] Права доступа файла логов корректны

```go
auditFile, _ := os.OpenFile("/var/log/app/audit.log",
    os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0600)
cfg.AuditEnabled = true
cfg.AuditHandler = env.NewJSONAuditHandler(auditFile)
```

### Обработка конфиденциальных данных

- [ ] Конфиденциальные значения получаются через `GetSecure`
- [ ] `Close()` вызывается своевременно для освобождения ресурсов
- [ ] В логи не выводятся исходные конфиденциальные значения

```go
secret := loader.GetSecure("DB_PASSWORD")
defer secret.Close()
log.Printf("Password length: %d", secret.Length())
```

### Управление доступом

- [ ] Установлен белый список `AllowedKeys` (рекомендуется)
- [ ] Включена `ValidateValues`
- [ ] Разумно установлены ограничения размера

```go
cfg.AllowedKeys = []string{"APP_NAME", "DB_HOST", "API_KEY"}
cfg.ValidateValues = true
cfg.MaxVariables = 100
```

## Проверки при развёртывании

- [ ] Файл конфигурации загружается из безопасного расположения
- [ ] При запуске приложения выполняется валидация конфигурации
- [ ] При ошибке конфигурации приложение отказывается запускаться
- [ ] Конфиденциальная информация не выводится в логи

## Проверки после развёртывания

- [ ] Приложение работает нормально
- [ ] Аудиторский журнал записывается корректно
- [ ] Нет утечки конфиденциальной информации
- [ ] Мониторинг отслеживает ошибки, связанные с конфигурацией

## Скрипт быстрой проверки

```bash
#!/bin/bash
# pre-deploy-check.sh

set -e

echo "=== Pre-deployment Config Check ==="

# Проверка существования файла
[ -f ".env.production" ] || { echo "ERROR: .env.production not found"; exit 1; }

# Проверка прав доступа
PERMS=$(stat -c %a .env.production 2>/dev/null || stat -f %Lp .env.production)
[ "$PERMS" = "600" ] || [ "$PERMS" = "400" ] || echo "WARNING: permissions are $PERMS"

# Проверка плейсхолдеров
grep -qE "(change-?me|placeholder|xxx|YOUR_)" .env.production && \
    { echo "ERROR: Found placeholder values"; exit 1; }

# Проверка обязательных ключей
for key in DB_HOST DB_PORT DB_USER DB_PASSWORD API_KEY; do
    grep -q "^$key=" .env.production || { echo "ERROR: Missing $key"; exit 1; }
done

echo "=== All checks passed ==="
```

## Связанная документация

- [Обзор безопасности](/ru/env/security/) - архитектура безопасности и основные функции
- [SecureValue API](/ru/env/api-reference/secure-value) - обработка безопасных значений
- [Константы и ошибки](/ru/env/api-reference/constants) - список запрещённых ключей
