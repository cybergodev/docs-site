...
:::info Требуется инициализация
Глобальный загрузчик по умолчанию должен быть явно инициализирован через `Load()` или `LoadWithConfig()` и **не** создаётся автоматически при первом вызове. Без инициализации функции ведут себя так:
...
if secret != nil {
    ...
    value := secret.Reveal()   // открытое значение (вызывать только при необходимости)
