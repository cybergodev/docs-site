---
title: Извлечение контента на практике - HTML
description: Практическое руководство по извлечению контента CyberGo HTML с подробным объяснением обзора процесса извлечения, принципов алгоритма интеллектуального распознавания статей, базового извлечения текста и файлов, полного описания полей структуры Result, реализации пользовательского скорера Scorer и лучших практик обработки не-UTF-8 кодировок с автоматическим определением и ручным указанием.
---
...
```text
HTML-ввод → Определение кодировки (автоконвертация в UTF-8) → Очистка безопасности → Парсинг DOM → Проверка глубины
    → Распознавание статьи (опционально) → Извлечение контента → Форматирование → Возврат Result
```
