...
description: "Шпаргалка по API CyberGo JSON: охватывает запросы по пути GetString/GetInt, операции изменения Set/Delete, сериализацию Marshal/Unmarshal, параметры конфигурации, итераторы и функции безопасности — удобный быстрый справочник для эффективной работы разработчиков."
---
...
| Массовая установка | `SetMultiple` | `json.SetMultiple(data, map[string]any{"a": 1, "b": 2})` |
| Установка с созданием пути | `SetCreate` | `json.SetCreate(data, "a.b.c", 1)` |
| Массовая установка с созданием пути | `SetMultipleCreate` | `json.SetMultipleCreate(data, updates)` |
...
fmt.Println("Равны:", equal) // true (игнорирует порядок и точность)
...
// Конфигурация безопасности (для обработки недоверенных данных)
...
```go
// Рекомендуемый способ: изменение на основе конфигурации по умолчанию
cfg := json.DefaultConfig()
cfg.MaxJSONSize = 200 * 1024 * 1024 // Пользовательский лимит размера
cfg.FullSecurityScan = true          // Включить полный скан безопасности
```
...
// Используйте Get, когда нужно различать типы ошибок
...
```go
// Функции GetString/GetInt и подобные поддерживают необязательный параметр значения по умолчанию
...
// После этого все функции уровня пакета будут использовать этот процессор
