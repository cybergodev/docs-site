---
title: Быстрый старт - CyberGo JSON | Руководство за 5 минут
description: "Руководство по быстрому старту CyberGo JSON: установка и настройка, запросы по пути GetString/GetInt, кодирование/декодирование Marshal/Unmarshal, операции чтения и записи файлов. Освойте лучшие практики работы с JSON в Go за 5 минут с поддержкой JSONPath-запросов и типобезопасного получения значений."
---

# Быстрый старт

Это руководство поможет вам быстро освоить библиотеку `github.com/cybergodev/json`.
...
Помимо базовых свойств и индексов массивов, поддерживаются **срезы массивов** `[1:5]`, **подстановочные знаки** `[*]`, **извлечение полей** `{name,email}` и другой расширенный синтаксис. Подробнее в [Синтаксис выражений пути](./path-syntax).
:::
...
// Конфигурация с повышенной безопасностью (для обработки недоверенных данных)
cfg = json.SecurityConfig()

// Конфигурация для форматированного вывода
...
err := json.ForeachWithPath(data, "users", func(key any, item *json.IterableValue) {
    ...
    age := item.GetInt("age")
    fmt.Printf("Пользователь %v: %s (возраст %d)\n", key, name, age)
})
// Пользователь 0: Alice (возраст 30)
// Пользователь 1: Bob (возраст 25)
```

## Что дальше

- [Синтаксис выражений пути](./path-syntax) — изучите полный синтаксис запросов по пути
- [Обработка больших файлов](./large-files) — работа с крупными JSON-файлами
