...
description: Полная документация API подписей целостности CyberGo DD, поддерживает алгоритм подписи HMAC-SHA256 и механизм отслеживания инкрементных серийных номеров, гарантирует, что каждая запись лога не была подделана, предоставляет подписывающее устройство IntegritySigner и верификатор Verify, удовлетворяющая различные требования соответствия аудиту безопасности и защиты логов от подделки.
---
...
Подписывающее устройство записей логов, поддерживает HMAC-подписи и цепочную проверку.
...
type IntegrityConfig struct {
    SecretKey       []byte         // HMAC-ключ
    HashAlgorithm   HashAlgorithm  // Алгоритм хеширования
    IncludeTimestamp bool           // Подпись включает временную метку
    IncludeSequence  bool           // Подпись включает серийный номер
    SignaturePrefix  string        // Префикс подписи
}
```
...
| `Clone` | `() IntegrityConfig` | Копировать конфигурацию |
| `MarshalJSON` | `() ([]byte, error)` | JSON-сериализация (ключ не сериализуется, выводится только длина) |
...
- [Константы и ошибки](./constants) -- коды ошибок
