...
`Field` — тип структурированного поля лога, содержит ключ (строка) и значение (любое значение), создаётся через конструкторы.

## Следующие шаги

- [Logger](../core/logger) -- методы WithFields / InfoWith
