...
description: CyberGo env 库 Config 配置结构体完整 API 参考文档，Config 用于控制 Loader 的全部行为，包括文件搜索路径、安全限制参数、键值验证选项、变量展开设置、审计日志配置和预定义配置模板，提供 Development 和 Production 预设，满足不同环境配置需求。
---
