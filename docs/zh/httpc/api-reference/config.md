...
type TimeoutConfig struct {
    Request        time.Duration // 总请求超时（含重试），默认 30s
    Dial           time.Duration // TCP 连接超时，默认 10s
    TLSHandshake   time.Duration // TLS 握手超时，默认 10s
    ResponseHeader time.Duration // 等待响应头超时，默认 30s
...
重试延迟公式：`Delay * BackoffFactor^attempt + jitter`
