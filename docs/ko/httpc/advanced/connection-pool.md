...
description: HTTPC 연결 풀과 프록시 구성 완전 가이드로, ConnectionConfig 연결 풀 매개변수 튜닝 방법, HTTP 정방향 프록시와 SOCKS5 프록시 구성, DNS-over-HTTPS 사용자 정의 해석기, 연결 누수 문제 해결 및 IdleConnTimeout 유휴 연결 관리 전략을 다룹니다.
---
...
연결 풀은 HTTP 클라이언트 성능의 핵심 요소입니다. HTTPC는 `ConnectionConfig`를 사용하여 연결 풀을 관리합니다.
...
| `IdleConn` | 90초 | 유휴 연결 타임아웃, 초과 시 종료 |
| `Dial` | 10초 | 연결 설정 타임아웃 |
| `TLSHandshake` | 10초 | TLS 핸드셰이크 타임아웃 |
| `ResponseHeader` | 30초 | 응답 헤더 대기 타임아웃 |

### 시나리오별 권장값

| 시나리오 | MaxIdleConns | MaxConnsPerHost | IdleConn |
|----------|-------------|-----------------|----------|
| 고동시성 API | 100 | 20 | 120초 |
| 일반 서비스 | 50 | 10 | 90초 |
| 저빈도 요청 | 10 | 2 | 30초 |
| 마이크로서비스 내부 | 50 | 10 | 60초 |

:::tip
`MaxConnsPerHost`는 활성 연결과 유휴 연결을 모두 포함합니다. 이 제한을 초과하는 새 요청은 연결이 해제될 때까지 대기합니다.
:::
...
| Google | `8.8.8.8/resolve` | 전 세계적 커버리지 |
...
| 문제 | 원인 | 해결 방법 |
|------|------|----------|
| 다수의 TIME_WAIT | 유휴 연결 타임아웃이 너무 짧음 | `IdleConn` 타임아웃 증가 |
...
- [구성 API](../api-reference/config) - 연결 구성 참조
