...
:::warning
알리바바 클라우드 메타데이터(`100.100.100.200`)는 CGNAT 범위(`100.64.0.0/10`)에 위치하며, Tailscale/WireGuard 등 VPN 지원을 위해 이 범위는 기본 차단 목록에 **포함되지 않습니다**. 알리바바 클라우드 메타데이터 접근을 방지하려면 다른 보안 정책(예: 방화벽 규칙)으로 제한하세요.
:::
