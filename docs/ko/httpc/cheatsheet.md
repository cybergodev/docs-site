---
title: "치트시트 - HTTPC"
description: "HTTPC 치트시트: 클라이언트 생성과 다섯 가지 프리셋 설정, Get/Post 등 일곱 가지 요청 메서드, 28개 WithXxx 요청 옵션, Result 응답 처리, 미들웨어 체인 조합, ClientError 오류 분류, 파일 다운로드, 이어받기와 도메인 클라이언트 작업의 완전한 코드 조각을 제공합니다."
---
