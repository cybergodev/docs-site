---
title: 미들웨어 - HTTPC
description: HTTPC 미들웨어 시스템의 전체 API 레퍼런스로, Chain 체인 조합 함수, Recovery, Logging, RequestID, Timeout, Header, Metrics, Audit 8개의 내장 미들웨어 팩토리 함수, AuditMiddlewareWithConfig 설정형 감사 및 AuditEvent 감사 이벤트 타입 정의를 포함합니다.
---

# 미들웨어

HTTPC는 양파 모델 미들웨어 아키텍처를 채택하여 `MiddlewareFunc`로 요청 처리 로직을 감쌉니다.
...
여러 미들웨어를 단일 미들웨어로 조합합니다. 전달된 순서대로 실행되며, 마지막 미들웨어가 처리를 마친 후 최종 Handler를 호출합니다.
...
panic 복구 미들웨어. 처리 체인에서 발생한 panic을 포착하여 스택 정보가 포함된 error로 변환하여 반환합니다.
...
요청 로깅 미들웨어. 메서드, URL, 상태 코드 및 소요 시간을 기록합니다. URL은 자동으로 마스킹됩니다 (자격 증명 정보 제거).
...
각 요청에 고유 ID를 추가합니다. 기본적으로 `crypto/rand`를 사용하여 32자 16진수 ID를 생성합니다.

| 매개변수 | 설명 |
|------|------|
| `headerName` | 요청 헤더 이름, 예: `"X-Request-ID"` |
| `generator` | 사용자 정의 ID 생성 함수, `nil`을 전달하면 기본 암호학적 안전 생성기 사용 |
...
:::tip
기본 생성기는 `crypto/rand`를 사용하여 예측 불가능한 ID를 생성하므로 보안에 민감한 시나리오에 적합합니다.
:::
...
미들웨어 수준의 타임아웃 제어. 클라이언트 내장 타임아웃 전에 적용되며, 타임아웃 시 컨텍스트를 취소하고 오류를 반환합니다.
...
각 요청에 정적 요청 헤더를 추가합니다. 생성 시점에 헤더 보안성을 검증합니다 (CRLF 인젝션 방지).
...
메트릭 수집 미들웨어. 매 요청 완료 후 콜백을 호출하여 메서드, URL, 상태 코드, 소요 시간 및 오류 정보를 전달합니다.
...
보안 감사 미들웨어로, 금융, 의료, 공공 등 규정 준수 시나리오에 적합합니다. 전체 요청/응답 정보를 기록하며, URL은 자동으로 마스킹됩니다.
...
| 필드 | 변환 규칙 |
|------|----------|
| `Duration` | `durationMs` 추가 (밀리초 정수), 기존 `duration` 필드 유지 (나노초) |
...
| 상수 | 타입 | 설명 |
|------|------|------|
| `SourceIPKey` | `auditContextKey` | 출발 IP 컨텍스트 키 |
