---
title: 패키지 함수 - HTTPC
description: HTTPC 패키지 함수와 클라이언트 메서드의 전체 API 레퍼런스로, Get, Post, Put, Patch, Delete, Head, Options 7가지 HTTP 메서드, New 클라이언트 생성 함수, Download 시리즈 다운로드 함수 및 ReleaseResult 객체 풀 재사용 메서드를 다룹니다.
---
...
클라이언트를 생성하지 않고 직접 요청을 보냅니다. 내부적으로 지연 초기화된 기본 클라이언트를 사용합니다.
...
Client 인터페이스는 패키지 함수와 동일한 HTTP 메서드에 더해 컨텍스트가 포함된 `Request` 메서드를 제공합니다.
...
새로운 HTTP 클라이언트를 생성합니다. 설정을 전달하지 않거나 `nil`을 전달하면 `DefaultConfig()`를 사용합니다.
...
Client 인터페이스 메서드로, 클라이언트가 보유한 리소스(연결 풀, Transport)를 해제합니다. 호출 후에는 재사용할 수 없습니다.
...
사용자 정의 클라이언트를 기본 클라이언트로 설정하여 패키지 함수에서 사용합니다. 기존 기본 클라이언트는 자동으로 닫힙니다.
...
Result를 객체 풀에 반환하여 GC 부담을 줄입니다. 호출 후 Result를 재사용할 수 없습니다.
