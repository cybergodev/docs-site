---
title: Result - HTTPC
description: HTTPC Result 응답 타입의 전체 API 레퍼런스로, Body와 RawBody 응답 본문 접근, StatusCode 상태 코드 조회, IsSuccess 등 상태 판단 메서드, Cookie 조작, Unmarshal JSON 자동 파싱 및 SaveToFile 파일 저장 메서드를 상세히 설명합니다.
---

# Result

Result는 HTTP 응답과 요청 메타데이터를 캡슐화하고 편리한 접근 메서드를 제공합니다. `Client.Request()` 또는 패키지 함수를 통해 얻습니다.
...
:::warning
사용 후 반드시 `ReleaseResult(result)`를 호출하여 객체 풀에 반환해야 합니다. 호출 후 Result에 다시 접근할 수 없습니다.
:::
...
JSON 응답 본문을 대상 변수로 파싱합니다. `json.Unmarshal` 규약을 따릅니다.
...
응답 본문을 파일로 저장합니다. 파일 경로는 보안 검증(경로 순회 방지, 심볼릭 링크 검사, 시스템 경로 보호)을 거칩니다.
...
사람이 읽을 수 있는 문자열 표현을 반환합니다. 민감한 헤더는 자동으로 마스킹되고, 응답 본문은 200자로 잘립니다.
...
Result를 객체 풀에 반환합니다. 응답 본문의 처음 64KB가 안전하게 지워지고, 모든 내부 데이터가 초기화됩니다. 호출 후 Result의 모든 필드나 메서드에 다시 접근할 수 없습니다.
