...
description: "CyberGo JSON 성능 최적화 가이드: 캐시 전략 EnableCache/CacheTTL, 병렬 처리 ParallelThreshold/MaxConcurrency, PreParse 사전 파싱, WarmupCache 예열, 객체 풀 재사용 및 벤치마크 분석을 상세히 다루어 고빈도 JSON 처리 성능을 전면적으로 향상시킵니다."
---
...
```go
// 패키지 수준 함수는 전역 Processor를 자동으로 재사용
for _, item := range dataList {
    val := json.GetString(item, "name")
}

// 또는 명시적으로 인스턴스 재사용 (커스텀 설정에 적합)
...
```go
// 자주 사용하는 경로 사전 컴파일 (Processor 사용)
p, err := json.New()
if err != nil {
    panic(err)
}
