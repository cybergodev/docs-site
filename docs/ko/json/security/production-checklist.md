...
description: "CyberGo JSON 프로덕션 환경 보안 배포 체크리스트: 보안 설정, MaxNestingDepthSecurity/MaxMemory 리소스 제한, 입력 검증, 오류 처리, 모니터링 알림 설정 및 성능과 보안의 균형을 위한 모범 사례를 다루어 Go 애플리케이션의 프로덕션 수준 JSON 처리 보안을 보장합니다."
---

# 프로덕션 체크리스트

프로덕션 환경에 배포하기 전에 다음 보안 항목을 확인하십시오.
...
if err != nil {
    ...
    return errors.New("작업 실패, 잠시 후 다시 시도하십시오")
}
```
...
func ProductionConfig() json.Config {
    ...
    // 리소스 제한 (SecurityConfig에 안전한 기본값이 미리 설정됨)
    cfg.MaxMemory = 100 * 1024 * 1024
