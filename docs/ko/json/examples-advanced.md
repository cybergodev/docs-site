...
description: "CyberGo JSON 고급 기능 실전 예제 모음으로, 배치 인코딩 EncodeBatch, 필드 선택 EncodeFields, 사전 파싱 PreParse, 안전 가져오기 SafeGet, 캐시 웜업 WarmupCache 및 메모리 풀 최적화 등의 기법을 다루며, 라이브러리의 고급 사용법과 프로덕션급 성능 최적화 전략을 보여줍니다."
---
...
func main() {
    ...
    // EncodeBatch를 PrettyConfig와 함께 포맷팅 출력
...
JSON을 사전 파싱하여 반복 파싱을 방지하고 여러 쿼리의 성능을 향상시킵니다:
...
구조화된 결과를 반환하며, 체인 호출 및 타입 변환을 지원합니다:
...
자주 사용하는 경로의 캐시를 미리 웜업하여 후속 쿼리 성능을 향상시킵니다:
...
라이브러리는 내부적으로 문자열 메모리 풀(string interning)을 사용하여 반복되는 키-값의 메모리 사용량을 자동으로 최적화합니다. 수동 관리가 필요하지 않습니다.
...
func main() {
    // 라이브러리는 내부적으로 반복되는 키-값에 대해 메모리 풀을 자동으로 사용합니다
    // 대량의 데이터를 처리할 때, 반복되는 문자열 키-값이 자동으로 메모리를 재사용합니다
    ...
    // 배치 인코딩 시 라이브러리가 내부적으로 메모리를 자동 최적화
...
- [경로 표현식 문법](./path-syntax) — 완전한 경로 문법 참조
