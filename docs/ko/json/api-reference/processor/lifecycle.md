---
title: Processor 수명 주기 - CyberGo JSON | API 참조
description: "CyberGo JSON Processor 수명 주기 관리 완전 참조: New 인스턴스 생성, Close 리소스 해제, IsClosed 상태 확인, Stats 통계 정보, HealthCheck 상태 모니터링 및 동시성 안전 종료와 리소스 보호 모범 사례 가이드를 포함합니다."
---
...
프로세서를 닫고 리소스를 해제합니다. Processor 사용 완료 후 이 메서드를 호출해야 합니다.
...
type HealthStatus struct {
    Timestamp time.Time              // 검사 시간
    Healthy   bool                   // 전체 상태
    Checks    map[string]CheckResult // 각 검사 결과
}
...
|------|------|------|
| `Timestamp` | `time.Time` | 검사 시간 |
| `Healthy` | `bool` | 전체 상태 정상 여부 |
| `Checks` | `map[string]CheckResult` | 각 검사 상세 정보 |
...
프로세서의 로거를 설정합니다. 디버깅 및 런타임 진단에 사용됩니다.
...
프로세서의 현재 설정 복사본을 가져옵니다. 반환된 설정은 프로세서에 영향을 주지 않고 안전하게 수정할 수 있습니다.
...
go func() {
    ...
    for range ticker.C {
        status := processor.GetHealthStatus()
        if !status.Healthy {
            log.Printf("Processor 상태 불량: %+v", status.Checks)
        }
    }
}()
```
