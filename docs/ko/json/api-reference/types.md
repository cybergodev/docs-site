---
title: 타입 정의 - CyberGo JSON | API 참조
description: "CyberGo JSON 핵심 타입 정의 완전 참조: Result[T] 제네릭 결과, AccessResult 동적 접근 결과, BatchOperation, BatchResult, Schema 검증 패턴, Stats, HealthStatus, IterableValue 및 인코딩 오류 타입을 포함합니다."
---

# 타입 정의

json 패키지는 JSON 작업 결과를 처리하기 위한 다양한 타입 안전성을 제공합니다.
...
| 메서드 | 시그니처 | 설명 |
|------|------|------|
| `Ok()` | `func (r Result[T]) Ok() bool` | 결과가 유효한지 확인 (오류 없고 발견됨) |
| `Unwrap()` | `func (r Result[T]) Unwrap() T` | 값을 반환, 실패 시 제로값 반환 |
...
func main() {
    ...
    // GetTyped를 사용하여 타입 지정 값 가져오기
    name := json.GetTyped[string](data, "user.name")
    fmt.Printf("이름: %s\n", name)

    // defaultValue 매개변수를 사용하여 기본값 제공
...
::: tip 명명 규칙
- **GetTyped[T]** - 지정된 타입의 값을 가져오고, `T`를 반환하며, `defaultValue` 매개변수를 지원합니다
- **Result[T]** - 정밀한 오류 처리가 필요한 시나리오를 위한 내부 결과 타입입니다
:::

---

## CompiledPath - 사전 컴파일된 경로

`CompiledPath`는 사전 컴파일된 JSON 경로 타입 별칭으로, 동일한 경로를 빈번하게 접근할 때 경로 문자열의 반복 파싱을 방지하여 성능을 향상시킵니다.
...
동일한 경로에 대해 대량의 반복 작업이 필요한 경우 (예: 루프에서 배치 쿼리), 경로를 미리 컴파일하여 매번 호출할 때마다 경로 문자열을 반복 파싱하는 것을 방지할 수 있습니다.
...
Processor를 통해 JSON 경로를 미리 컴파일하고, 후속 작업에서 재사용할 수 있는 `*CompiledPath` 인스턴스를 반환합니다.
...
// 후속 작업에서 compiled를 반복 사용 가능
val, err := processor.GetCompiled(data, compiled)
```

::: tip 성능 팁
고빈도 반복 경로 접근의 경우, 사전 컴파일된 경로를 사용하면 경로 파싱 오버헤드를 크게 줄일 수 있습니다. 배치 작업, 루프 쿼리 등의 시나리오에 적합합니다.
:::
...
`AccessResult`는 안전한 속성 접근 결과로, 체인 형식의 타입 변환을 제공합니다.
...
안전하게 속성을 가져오고, 체인 형식의 타입 변환을 위한 `AccessResult`를 반환합니다.

패키지 수준 함수 `SafeGet`도 사용할 수 있습니다:
...
| 메서드 | 반환 타입 | 설명 |
|------|----------|------|
| `Unwrap()` | `any` | 값을 반환, 존재하지 않을 때 nil 반환 |
| `UnwrapOr(defaultValue)` | `any` | 값 또는 기본값 반환 |
| `AsString()` | `(string, error)` | 문자열로 변환 (엄격 타입 검사) |
| `AsStringConverted()` | `(string, error)` | 포맷하여 문자열로 변환 |
| `AsInt()` | `(int, error)` | 정수로 변환 (bool은 변환하지 않음) |
| `AsFloat64()` | `(float64, error)` | float64로 변환 (bool은 변환하지 않음) |
| `AsBool()` | `(bool, error)` | 불리언으로 변환 |

::: warning 주의
`AsInt64()`, `AsArray()`, `AsObject()` 메서드는 제거되었습니다. 이러한 타입을 가져오려면 `GetTyped[T]`를 사용하십시오.
:::
...
// 배열이나 객체 타입이 필요할 때는 GetTyped 사용
...
type BatchResult struct {
    ID     string // 작업 식별자 (BatchOperation.ID에 대응)
    Result any    // 작업 결과
    Error  error  // 오류 (있는 경우)
}
```
...
type WarmupResult struct {
    TotalPaths  int      // 총 경로 수
    Successful  int      // 성공한 웜업 수
    Failed      int      // 실패 수
...
| 시나리오 | 설명 |
|------|------|
| 고빈도 쿼리 | 동일한 JSON에 대한 여러 쿼리 시 반복 파싱 방지 |
| 배치 경로 가져오기 | `GetMultiple`을 사용하여 여러 경로를 배치로 가져오기 |
| 성능 최적화 | 사전 파싱 후 쿼리 성능이 크게 향상됨 |

::: tip 성능 팁
동일한 JSON 문자열을 여러 번 쿼리해야 하는 시나리오에서 `PreParse` 사전 파싱을 사용하면 반복 파싱 오버헤드를 피하여 성능을 크게 향상시킬 수 있습니다.
:::
...
| `IsEmpty(path)` | 지정된 경로가 비어 있는지 확인 |
| `Break()` | 반복 중단 신호 반환, 반복 중지 |

자세한 내용은 [이터레이터](./iterator) 문서를 참조하십시오.

---

## 인코딩 오류 타입

json 패키지는 인코딩/디코딩 과정에서의 정밀한 오류 처리를 위해 다음 오류 타입을 내보냅니다.

### SyntaxError - 구문 오류

JSON 구문 파싱 오류로, 입력 데이터가 유효한 JSON 형식이 아님을 나타냅니다.
...
### InvalidUnmarshalError - 잘못된 역직렬화 대상 오류

`Unmarshal`의 대상 매개변수가 포인터가 아니거나 nil일 때 이 오류가 반환됩니다.
...
| 메서드 | 시그니처 | 설명 |
|------|------|------|
| `Error` | `func (e *InvalidUnmarshalError) Error() string` | 잘못된 대상에 대한 오류 설명 반환 |
...
`w`에 쓰는 인코더를 생성합니다. 선택적 `Config` 매개변수를 통해 인코딩 동작을 사용자 정의할 수 있습니다.
...
| `SetEscapeHTML` | `func (enc *Encoder) SetEscapeHTML(on bool)` | HTML 특수 문자 이스케이프 설정 |
...
| `UseNumber` | `func (dec *Decoder) UseNumber()` | 디코더가 숫자를 `float64` 대신 `Number`로 파싱하도록 설정 |
