---
title: 조회와 가져오기 함수 - CyberGo JSON | API 참조
description: "CyberGo JSON 조회와 가져오기 함수 완전 참조: Get/GetString/GetInt/GetFloat/GetBool 등 타입 안전 가져오기, GetTyped[T] 제네릭 가져오기 및 Parse/ParseAny 파싱 함수를 포함하여 JSONPath 경로 표현식을 완전히 지원합니다."
---
...
타입 안전 가져오기 함수는 `defaultValue` 가변 매개변수를 통해 제로값 대체를 제공합니다. 경로가 존재하지 않거나, 값이 null이거나, 타입 변환에 실패하면 `defaultValue`를 반환합니다 (제공하지 않으면 해당 타입의 제로값 반환).
...
func main() {
    ...
    // 존재하지 않는 경로는 제로값(빈 문자열) 또는 사용자 정의 기본값 반환
...
제네릭 가져오기 함수로, 사용자 정의 타입을 지원합니다. 경로가 존재하지 않거나 타입 변환에 실패하면 `defaultValue`를 반환합니다 (제공하지 않으면 `T`의 제로값 반환).

**명명 규칙 안내**: `GetTyped[T]`는 `GetAs[T]` 의미와 동일하며, JSON 값을 가져와 지정된 타입 `T`로 변환함을 나타냅니다.
...
JSON 문자열을 파싱하여 루트 값을 `any` 타입으로 반환합니다. 대상 변수를 미리 선언할 필요가 없습니다.
...
Processor 인스턴스를 통해 JSON을 파싱하고 `any` 타입으로 반환합니다. 패키지 수준 `ParseAny`와 동일한 동작을 합니다.
...
자세한 내용은 [Processor 파싱 메서드](../processor/parse.md#파싱-메서드)를 참조하십시오.
...
타입 안전한 가져오기 작업을 수행하여 `AccessResult`를 반환합니다. 타입 변환 메서드(`AsString`, `AsInt`, `AsFloat64`, `AsBool`)를 제공합니다.
...
다음 메서드는 패키지 수준 함수와 Processor 메서드로 모두 제공됩니다.

### GetMultiple (패키지 수준 함수)

시그니처: `func GetMultiple(jsonStr string, paths []string, cfg ...Config) (map[string]any, error)`

여러 경로의 값을 배치로 가져옵니다 (패키지 수준 함수, Processor 생성 불필요).
...
`SafeGet`에서 사용하는 `AccessResult` 구조체 필드:
...
- [보조 함수](../helpers) - CompareJSON, MergeJSON 등 도구 함수
