---
title: 파일 작업 함수 - CyberGo JSON | API 참조
description: "CyberGo JSON 파일 작업 함수 완전 참조: LoadFromReader 스트림 읽기, ParseJSONL/ToJSONL JSONL 처리, StreamLinesInto[T] 제네릭 스트림 처리, NewJSONLWriter 쓰기 및 JSONL 설정 상세 설명을 포함합니다."
---

# 파일 작업 함수

json 패키지가 제공하는 파일 작업과 JSONL 처리 함수입니다.
...
io.Reader에서 JSON 데이터를 로드합니다. 네트워크 연결, HTTP 요청 본문 등 스트림 데이터 소스에서 JSON을 읽는 데 적합합니다.
...
JSONL(JSON Lines)은 줄바꿈으로 구분된 JSON 형식으로, 한 줄에 하나의 독립적인 JSON 객체가 있습니다.
...
io.Reader에서 JSONL 데이터를 스트림으로 읽어 콜백 함수로 각 줄을 처리합니다. 권장되는 제네릭 JSONL 처리 방식입니다.
...
::: warning
`JSONLConfig` 독립 구조체와 `DefaultJSONLConfig()` 함수는 제거되었습니다. JSONL 설정은 `Config`의 `JSONL*` 필드에 통합되었습니다.
:::
...
cfg.JSONLMaxLineSize   = 1024 * 1024  // 단일 줄 최대 크기 (기본값: 1MB)
...
자세한 내용은 [Config 설정](../config#config-구조체)을 참조하십시오.
...
| `WriteRaw` | `(line []byte) error` | 원본 바이트 줄 쓰기 |
| `Err` | `() error` | 누적된 오류 반환 |
...
- [스트림 처리](../../large-files) - 스트림 프로세서 상세 설명
- [Processor JSONL 메서드](../processor/jsonl) - Processor 수준 JSONL 메서드 상세 설명
