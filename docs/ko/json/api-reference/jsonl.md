---
title: JSONL 처리기 - CyberGo JSON | API 참조
description: "CyberGo JSON JSONL/NDJSON 처리기 참조: StreamJSONL 스트림 처리, JSONLWriter 쓰기, StreamLinesInto[T] 제네릭 스트림, ParseJSONL 파싱, ToJSONL 변환 및 설정 옵션을 포함하여 JSON Lines 형식 읽기/쓰기와 고급 작업을 완전히 지원합니다."
---

# JSONL 처리기

JSONL(JSON Lines) 또는 NDJSON(Newline Delimited JSON)은 한 줄에 하나의 JSON 객체가 있는 형식입니다. 이 라이브러리는 `Processor` 메서드와 패키지 수준 함수를 통해 완전한 JSONL 처리 기능을 제공합니다.
...
JSONL 데이터를 스트림으로 처리하며, 각 줄마다 `IterableValue`를 반환합니다.
...
err = p.StreamJSONL(file, func(lineNum int, item *json.IterableValue) error {
    ...
    age := item.GetInt("age")
    fmt.Printf("줄 %d: name=%s, age=%d\n", lineNum, name, age)
    return nil // 계속 처리
    // return item.Break() // 반복 중단
})
```
...
작업 풀 모드를 사용하여 JSONL 데이터를 병렬 처리합니다.
...
|------|------|------|
| `reader` | `io.Reader` | 데이터 소스 |
| `workers` | `int` | 작업 고루틴 수 (<=0이면 기본값 4) |
| `fn` | `func(lineNum int, item *IterableValue) error` | 처리 콜백 |
...
::: tip 성능 팁
CPU 집약적 작업(예: 데이터 변환, 계산)에는 병렬 처리를 사용하면 성능이 크게 향상됩니다. I/O 집약적 작업에는 단일 스레드 처리를 권장합니다.
:::
...
JSONL 데이터를 분배 처리하며, 매번 지정한 수의 요소를 처리합니다.
...
JSONL 쓰기기는 데이터를 JSON Lines 형식으로 쓰는 데 사용됩니다.
...
원본 JSON 줄을 씁니다 (JSON 인코딩을 수행하지 않음).
...
`map[string]any` 타입을 전문으로 처리하는 NDJSON 파일 처리기입니다.

### NewNDJSONProcessor

시그니처: `func NewNDJSONProcessor(cfg ...Config) *NDJSONProcessor`

NDJSON 처리기를 생성합니다. 선택적 설정 매개변수를 지원합니다.
...
패키지 수준 함수로, Processor를 생성하지 않고 파일에서 직접 JSONL 데이터를 스트림 처리합니다.
...
- [대용량 파일 처리 API](./large-file) - ForeachFile 계열 메서드
- [대용량 파일 처리 가이드](../large-files) - 대용량 파일 처리 가이드
- [반복기](./iterator) - 반복 순회 API
