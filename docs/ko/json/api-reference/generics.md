---
title: 제네릭 작업 - CyberGo JSON | API 참조
description: "CyberGo JSON 제네릭 API 완전 참조: GetTyped[T] 제네릭 가져오기 함수, Result[T] 제네릭 결과 타입, AccessResult 동적 타입 접근 및 타입 안전 작업 가이드를 상세히 설명하며, Go 1.18+ 제네릭 기능을 활용하여 컴파일 타임 타입 안전 검사를 구현하고 런타임 타입 오류를 줄입니다."
---

# 제네릭 작업

json 라이브러리는 Go 1.18+ 제네릭 기능을 활용한 타입 안전 작업을 제공하여 컴파일 타임 타입 검사를 구현합니다.

## GetTyped

시그니처: `func GetTyped[T any](jsonStr, path string, defaultValue ...T) T`

JSON에서 지정된 타입의 값을 가져옵니다. 사용자 정의 타입을 지원합니다. `T`를 반환하며 error가 없습니다. 경로가 존재하지 않거나 타입 변환에 실패하면 제로값 또는 `defaultValue`로 지정된 기본값을 반환합니다.
...
| `defaultValue` | `...T` | 아니오 | 선택적 기본값, 경로가 존재하지 않거나 타입 변환에 실패하면 반환 |

**반환값**

| 반환값 | 타입 | 설명 |
|--------|------|------|
| 유일한 반환값 | `T` | 가져온 값, 경로가 존재하지 않거나 타입 변환에 실패하면 제로값 또는 기본값 반환 |
...
안전하게 정수로 변환합니다. 모든 정수 타입과 float(정수값인 경우)을 지원합니다. **참고: bool은 int로 변환되지 않습니다.**

#### AsFloat64

시그니처: `func (r AccessResult) AsFloat64() (float64, error)`

안전하게 부동소수점으로 변환합니다. 모든 숫자 타입을 지원합니다. **참고: bool은 float64로 변환되지 않습니다.**
...
| 메서드 | 동작 | 사용 시나리오 |
|------|------|----------|
| `AsString()` | 엄격 타입 검사, string 타입만 성공 | 원래 타입을 보장해야 하는 경우 |
| `AsStringConverted()` | 임의의 타입을 문자열로 형식 변환 | 문자열 표현이 필요한 경우 |
...
제네릭 작업은 런타임에 리플렉션을 사용하여 타입 변환을 수행하므로, 타입별 getter(`GetString`, `GetInt` 등)보다 약간 느립니다. 성능에 민감한 시나리오에서는 타입별 함수를 사용하는 것이 좋습니다.

| 메서드 | 성능 | 추천 시나리오 |
|------|------|----------|
| `GetString`, `GetInt` 등 | 가장 빠름 | 성능 민감, 타입이 알려진 경우 |
| `GetTyped[T]` | 보통 | 사용자 정의 타입 필요 |
| `SafeGet` + `AccessResult` | 보통 | 동적 타입 처리 |
...
| 메서드 | 반환 타입 | 설명 |
|------|----------|------|
| `Ok()` | `bool` | 결과가 유효한지 확인 (오류가 없고 찾음) |
| `Unwrap()` | `T` | 값을 반환, 실패 시 제로값 반환 |
| `UnwrapOr(default T)` | `T` | 값을 반환, 실패 시 기본값 반환 |
...
|------|-----------|---------------------|
| 타입 안전성 | 제네릭 T | any 타입 |
| 존재 확인 | `Exists bool` | `Exists bool` |
| 오류 처리 | 내장 Error 필드 | 타입 변환 메서드가 error 반환 |
| 체인 호출 | 미지원 | 체인 타입 변환 지원 |
| 가져오기 방식 | `GetTyped[T]` | `SafeGet()` |
| 적용 시나리오 | 타입을 아는 경우 | 동적 타입 처리 |

### 선택 가이드

- **타입을 아는 경우**: `Result[T]`와 `GetTyped[T]` 사용
...
- [패키지 함수](./functions) - 타입별 getter 함수
