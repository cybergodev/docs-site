...
cfg가 없으면 기존 동작과 동일합니다 (보안 검증을 수행하지 않고, 양쪽 모두 `encoding/json`으로 마샬링). cfg를 전달하면 두 입력에 보안 검증(크기/깊이/위험 모드 제한)을 적용하고, 설정의 인코딩으로 대칭 비교를 수행합니다.
...
:::warning API 변경 안내
Print, PrintPretty, PrintE, PrintPrettyE는 라이브러리에서 제거되어 더 이상 제공되지 않습니다. 대신 [EncodeWithConfig](./functions/output#encodewithconfig), [EncodePretty](./functions/output#encodepretty) 또는 [Prettify](./functions/output#prettify)와 함께 `fmt.Println`을 사용하세요 (`Encode`는 더 이상 사용되지 않음). 자세한 내용은 [출력 함수](./print)를 참조하세요.
:::
