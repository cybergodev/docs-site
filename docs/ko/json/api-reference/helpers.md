---
title: 보조 함수 - CyberGo JSON | API 참조
description: "CyberGo JSON 보조 도구 함수 완전 참조: CompareJSON으로 두 JSON이 동등한지 비교 검증, ClearCache/GetStats 캐시 관리와 통계, 전역 프로세서 관리 및 보안 패턴 보조 함수를 포함하여 일상 개발에서 편리한 JSON 도구 함수 모음을 제공합니다."
---
...
두 JSON 객체를 병합합니다. Config로 병합 모드를 설정할 수 있습니다. 자세한 내용은 [수정 함수](./functions/modify#mergejson)를 참조하십시오.
...
여러 JSON 객체를 병합합니다. 자세한 내용은 [수정 함수](./functions/modify#mergemany)를 참조하십시오.

---

## 캐시와 통계

### ClearCache (패키지 수준 함수)
...
전역 프로세서는 모든 패키지 수준 함수(예: `Get`, `GetString` 등)에 사용됩니다.

### SetGlobalProcessor

시그니처: `func SetGlobalProcessor(processor *Processor)`

사용자 정의 전역 프로세서를 설정합니다.
...
// 이후 모든 패키지 수준 함수가 이 프로세서를 사용합니다
...
::: warning API 변경 안내
`Print`, `PrintPretty`, `PrintE`, `PrintPrettyE`는 내부 함수(소문자 명명)로 전환되어 공개 API로 내보내지지 않습니다. 대신 [Encode](./functions/encode-decode#encode), [EncodePretty](./functions/encode-decode#encodepretty) 또는 [Prettify](./functions/encode-decode#prettify)와 함께 `fmt.Println`을 사용하십시오. 자세한 내용은 [출력 함수](./print)를 참조하십시오.
:::
...
Config의 `AddDangerousPattern` 메서드 또는 `AdditionalDangerousPatterns` 필드를 통해 사용자 정의 위험 패턴을 등록합니다.
...
cfg.AdditionalDangerousPatterns = []json.DangerousPattern{
    {Pattern: "malicious_keyword", Name: "사용자 정의 악성 키워드", Level: json.PatternLevelCritical},
}
p, err := json.New(cfg)
if err != nil {
    panic(err)
...
`AccessResult`은 `Processor.SafeGet()` 및 패키지 수준 `SafeGet()`의 반환 타입으로, 타입 안전한 변환 메서드를 제공합니다.
...
- [조회 가져오기 함수](./functions/get) - Get, GetString 등 조회 작업
