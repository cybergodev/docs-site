...
description: "CyberGo DD 라이프사이클 훅 시스템 전체 API 문서. 로그 기록 전후(BeforeLog/AfterLog), 파일 순환(OnRotate), 오류 발생(OnError) 등 핵심 이벤트에 커스텀 콜백을 등록할 수 있으며, HookRegistry 레지스트리와 유연한 로그 처리 확장 메커니즘을 제공합니다."
sidebar_position: 1
---
...
- 다른 이벤트에서 반환된 오류는 기본적으로 후속 훅 실행을 중단시킵니다; 오류 핸들러(참조: `HookErrorHandler`)를 설정한 경우에는 오류를 핸들러로 넘기고 모든 훅이 여전히 실행됩니다.
- 훅 내부에서 발생한 panic은 `HookRegistry`가 캡처하여 오류로 변환하므로 애플리케이션 전체가 중단되지 않습니다.
...
- `hookCtx`: 훅에 전달된 컨텍스트
- `err`: 훅이 반환한 (또는 panic에서 변환된) 오류

오류 핸들러를 설정한 후(`HookRegistry.SetErrorHandler` 또는 `HooksConfig.ErrorHandler`를 통해)에는 `Trigger`가 모든 훅을 실행하며 오류 발생 시 즉시 중단하지 않습니다; 각 오류는 핸들러에 전달됩니다. **예외**: `BeforeLog` 이벤트의 경우, 핸들러를 설정했더라도 반환된 오류는 여전히 로그 기록을 차단합니다. 핸들러 내부에서 panic이 발생해서는 안 됩니다; panic이 발생하면 복구되어 stderr로 출력됩니다.
