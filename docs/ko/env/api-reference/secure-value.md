---
title: SecureValue API - CyberGo env | 보안 값 저장소
description: CyberGo env 라이브러리 SecureValue 보안 값 타입 완전한 API 레퍼런스 문서로, SecureValue는 메모리 잠금 보호로 디스크 스왑 방지, 자동 초기화 소멸, 데이터 마스크 처리, 민감 키 이름 자동 감지 및 보안 도구 함수를 제공하여 비밀번호, API 키 및 액세스 토큰 등 Go 애플리케이션 민감 데이터를 안전하게 저장합니다.
---

# SecureValue API

`SecureValue` 타입은 민감 데이터를 안전하게 저장하며, 메모리 잠금, 자동 초기화 및 마스크 기능을 제공합니다.

## 스레드 안전

`SecureValue`의 모든 메서드는 스레드 안전하며, 여러 goroutine에서 동시에 사용할 수 있습니다:

- **읽기 메서드** (`String()`, `Bytes()`, `Length()`, `Masked()`)는 읽기 락을 사용하여 동시 읽기 지원
- **닫기 메서드** (`Close()`, `Release()`)는 쓰기 락을 사용하여 안전한 초기화 보장
- **상태 확인** (`IsClosed()`, `IsMemoryLocked()`)는 원자적 연산 사용
...
**동작:**
- 객체 풀을 사용하여 할당 감소
- GC 파이널라이저 설정으로 자동 초기화
- 메모리 잠금이 활성화된 경우 잠금 시도 (실패 시 조용히 무시)
...
보안 값을 생성하며, 메모리 잠금에 실패하면 오류를 반환합니다.
...
**반환값:**
- `string` - 값의 복사본, 닫힌 경우 빈 문자열 반환

```go
secret := env.GetSecure("PASSWORD")
if secret != nil {
    value := secret.String()  // 복사본 생성
    // value 사용
}
```
...
값의 바이트 슬라이스 복사본을 반환합니다. 호출자는 `ClearBytes`를 사용하여 초기화할 책임이 있습니다.
...
if secret != nil {
    data := secret.Bytes()
    defer env.ClearBytes(data)  // 사용 후 초기화
    // data 사용
}
```
...
if secret != nil {
    defer secret.Release()
    // Release 후 메모리가 초기화되고 객체가 풀에 반환됨
}
```

::: tip Close vs Release
- `Close()` - 초기화만, 풀에 반환하지 않음
- `Release()` - 초기화 후 풀에 반환 (고빈도 시나리오에 권장)
:::
...
**반환값:**
- `error` - 잠금 오류, 성공했거나 시도하지 않은 경우 nil
...
전역적으로 메모리 잠금을 활성화/비활성화합니다. 새로 생성되는 모든 SecureValue에 영향을 줍니다.
...
엄격 모드를 설정합니다. 활성화하면 `NewSecureValueStrict`에서 잠금 실패 시 오류를 반환합니다.
...
바이트 슬라이스를 안전하게 초기화합니다. 민감 데이터를 사용 후 즉시 초기화합니다.
...
문자열에서 잠재적으로 민감한 콘텐츠를 마스크합니다. 50자를 초과하는 문자열은 잘립니다.
...
::: tip 사용 시나리오
민감 데이터가 포함될 수 있는 긴 문자열을 자르는 데 사용합니다. 민감 키-값 쌍을 자동으로 마스크하려면 `SanitizeForLog`를 사용하세요.
:::
...
생성 시 GC 파이널라이저를 설정하여 가비지 컬렉션 시 자동 초기화를 보장합니다:
...
`unsafe.Pointer`를 사용하여 컴파일러 최적화를 방지합니다:
...
- [상수 및 오류](/ko/env/api-reference/constants) - 금지 키, 민감 키 패턴, 오류 타입
