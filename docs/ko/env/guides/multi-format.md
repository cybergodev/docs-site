...
description: CyberGo env 라이브러리 다중 형식 설정 파일 로딩 완전 사용 가이드입니다. .env, JSON, YAML 세 가지 설정 형식의 자동 감지, 혼합 로딩 및 키-값 병합, 파일 우선순위 제어, 디렉토리 재귀 스캔 로딩 및 형식 간 상호 변환 기능을 지원하여 Go 개발자가 다양한 프로젝트에서 환경 설정을 유연하게 관리하고 전환할 수 있도록 돕습니다.
---

# 다중 포맷 설정

env 라이브러리는 `.env`, JSON, YAML 세 가지 설정 형식을 지원하며, 형식을 자동 감지하여 로드할 수 있습니다.
...
// 저장됨: DATABASE_HOST=localhost, DATABASE_PORT=5432
...
| 입력 키 이름 | 변환 결과 |
|----------|--------|
| `"DATABASE_HOST"` | `"DATABASE_HOST"`（정확히 매칭） |
| `"database.host"` | `"DATABASE_HOST"`（점을 밑줄로 변환） |
| `"app.config.name"` | `"APP_CONFIG_NAME"` |
| `"servers.0.host"` | `"SERVERS_0_HOST"`（배열인덱스） |

::: tip 권장 사용법
- **코드에서 평탄화 키 이름 사용**：`GetString("DATABASE_HOST")` - 명확하고 효율적
- **설정 파일에서 가독성 있는 경로**：JSON/YAML에서 자연스러운 중첩 구조 사용
:::
...
```go
// YAMLNumberAsString = true（기본）
port := loader.GetString("PORT")  // "8080"（문자열）
port := loader.GetInt("PORT")     // 8080（정수）

// YAMLBoolAsString = true（기본）
debug := loader.GetString("DEBUG")  // "true"（문자열）
debug := loader.GetBool("DEBUG")    // true（불리언）
...
```bash
# export 접두사 지원 (AllowExportPrefix = true인 경우)
...
config/
├── base.json          # 기본 설정
├── development.env    # 개발덮어쓰기
├── production.yaml    # 프로덕션덮어쓰기
...
```text
명령줄 인수 > 환경 변수 > local 설정 > 환경 설정 > base 설정
```
...
자세한 내용 [커스텀 파서](/ko/env/guides/custom-parser)。
...
- [직렬화](/ko/env/guides/serialization) - 직렬화/역직렬화 상세
- [ComponentFactory API](/ko/env/api-reference/factory) - 형식 감지와 파서 등록
- [커스텀 파서](/ko/env/guides/custom-parser) - 커스텀 형식 추가
- [Config API](/ko/env/api-reference/config) - JSON/YAML 파싱설정
