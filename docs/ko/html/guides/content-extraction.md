...
description: CyberGo HTML 콘텐츠 추출 실전 가이드, 추출 프로세스 개요, 문서 스마트 인식 알고리즘 원리, 기본 텍스트 추출과 파일 추출, Result 구조 필드 완전 해석, 커스텀 Scorer 스코어러 구현 방법, 비 UTF-8 인코딩 자동 감지와 수동 지정의 모범 사례를 상세히 설명합니다.
---
...
```text
HTML 입력 → 인코딩 감지(자동 UTF-8 변환) → 보안 정제 → DOM 파싱 → 깊이 검증
    → 문서 인식(선택) → 콘텐츠 추출 → 형식화 → Result 반환
```
