...
:::tip 참고
이 예시의 `strings.Contains`는 표준 라이브러리 `strings` 패키지에서 가져옵니다. 완전한 실행 가능한 예시는 [테스트와 커스텀 확장](../integration/testing-custom)을 참조하세요.
:::
...
- [출력 형식 선택](./output-formats) - 시나리오에 맞는 출력 형식 선택
- [Processor 재사용과 캐시](../advanced-patterns/processor-cache) - 고빈도 호출의 성능 최적화
