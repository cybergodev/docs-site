...
description: CyberGo env 環境変数管理ライブラリのよく使う API チートシート。設定ファイルの読み込み、型安全な読み取り、変数の検証フィルタリング、セキュア値 SecureValue ストレージ、シリアライズデシリアライズ、構造体マッピング、監査ログなどの高頻度操作のコアコードスニペットを1ページにまとめ、Go 開発者が日常開発時に素早く参照してコピーできるようにします。
---

# チートシート

このライブラリについて理解した上で、高頻度で使用するコードスニペットを素早く参照できます。

## 設定の読み込み

```go
// パッケージレベルで読み込み
env.Load(".env")                                        // .env ファイルの読み込み
...
- [パッケージ関数](/ja/env/api-reference/functions) - 詳細 API
