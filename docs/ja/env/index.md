---
title: env - 環境変数管理
description: CyberGo env は高セキュリティの Go 環境変数管理ライブラリで、.env、JSON、YAML の複数フォーマットファイルの読み込みと自動検出をサポートし、型安全な変換、スレッドセーフな並行アクセス、メモリ安全なストレージ保護、完全な監査ログ機能を提供し、Web サービス、CLI ツール、コンテナ化デプロイなどの Go アプリケーションシーンに適しています。
---
