...
description: "CyberGo JSON コア型定義完全リファレンス：Result[T] ジェネリック結果、AccessResult 動的アクセス結果、BatchOperation、BatchResult、Schema バリデーションスキーマ、Stats、HealthStatus、IterableValue、エンコードエラー型を含む。"
---

# 型定義

json パッケージは JSON 操作結果を処理するための複数の型安全な型を提供します。

## Result[T] - 統一結果型

`Result[T]` はジェネリック操作結果型で、型安全なエラー処理と値アクセスを提供します。
...
事前解析済みの JSON ドキュメント。複数回のクエリ操作で再利用可能です。
...
| シナリオ | 説明 |
|------|------|
| 高頻度クエリ | 同一 JSON の複数回クエリで再解析を回避 |
| 一括パス取得 | `GetMultiple` で複数パスを一括取得 |
| パフォーマンス最適化 | 事前解析後のクエリパフォーマンスが大幅に向上 |

::: tip パフォーマンスのヒント
同じ JSON 文字列に対して複数回クエリする必要がある場面では、`PreParse` による事前解析で再解析のオーバーヘッドを大幅に削減できます。
:::
