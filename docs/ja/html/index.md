...
description: CyberGo HTML は Go 言語の高性能 HTML コンテンツ抽出・クリーニングツールライブラリで、スマート記事認識、自動エンコーディング検出（15+ 種類）、独立リンク抽出、並列バッチ処理、プラグイン可能な監査パイプラインを提供し、プレーンテキスト、Markdown、JSON のマルチフォーマット出力をサポートし、Web コンテンツスクレイピング、データクリーニング、ドキュメント分析などのプロダクション用途に適しています。
---
