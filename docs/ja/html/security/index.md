---
title: セキュリティ概要 - HTML
description: CyberGo HTML ライブラリのセキュリティ保護概要。入力サイズ制限（デフォルト 50MB）、DOM 深度制限（デフォルト 500）、パストラバーサル防御、パニックリカバリメカニズム、処理タイムアウト制御、コンテンツクリーニング機能、プラグイン可能な監査システムを含み、すべてのセキュリティ違反は errors.Is と errors.As によるエラー判定をサポートし、プロダクション環境での安全な実行を確保します。
---
