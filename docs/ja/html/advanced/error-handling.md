---
title: エラー処理 - HTML
description: CyberGo HTML ライブラリのエラー処理完全ガイド。5 種類のエラー分類（入力エラー、設定エラー、ファイルエラー、処理エラー、システムエラー）、errors.Is センチネルエラー判定パターン、errors.As 構造化エラー情報の抽出、context コンテキストキャンセルの処理、バッチ処理の部分失敗処理パターンを含み、堅牢なエラー処理ロジックの構築をサポートします。
---
