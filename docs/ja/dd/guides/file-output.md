...
description: "CyberGo DD ファイル出力とログローテーション設定ガイド。FileWriter のサイズローテーションと時間クリーンアップ戦略、BufferedWriter バッファ最適化、MultiWriter マルチ出力先ディスパッチ、動的 Writer 管理、本番ベストプラクティスで高信頼性ファイルログシステムを構築。"
sidebar_position: 3
---
...
:::info 圧縮とバックアップは共存しない
`Compress` を有効にすると、圧縮はローテーション後に別の goroutine で非同期に行われます；圧縮完了時に元の `.log` バックアップは `.log.gz` に**リネーム**され、両者は共存しません。
:::
