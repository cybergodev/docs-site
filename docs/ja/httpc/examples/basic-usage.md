---
title: 基本的な使い方 - HTTPC
description: HTTPC 基本的な使い方の完全なサンプル集。GET と POST リクエストの送信と JSON レスポンス解析、Multipart ファイルアップロード、クライアント設定プリセットの選択、ミドルウェアチェーンの追加、大きなファイルのプログレスコールバック付きダウンロードなど、一般的な開発シナリオの完全な実行可能な Go コード例と詳細な使用方法とコメント。
---
