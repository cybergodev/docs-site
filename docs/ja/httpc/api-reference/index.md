...
description: HTTPC API リファレンスドキュメントのインデックスページ。公開パッケージ関数、Client インターフェースメソッド、26 個のリクエストオプション関数、組み込みミドルウェアシステム、エラータイプ、設定構造体、定数定義を機能モジュール別に分類し、Go 開発者が必要なインターフェース定義と使用方法を素早く見つけられるよう支援します。
---
...
| モジュール | 説明 |
|------------|------|
| [リクエストオプション](./options) | 26 個の WithXxx リクエストオプション関数（ヘッダー、ボディ、認証、Cookie、コールバックなど） |
