...
分類された HTTP クライアントエラー。`errors.As` で抽出。
...
| フィールド | タイプ | 説明 |
|------------|--------|------|
| `Type` | `ErrorType` | エラー分類。switch 判断に使用 |
...
| `StatusCode` | `int` | HTTP ステータスコード（HTTP エラー以外は 0） |
...
| `Code()` | `string` | 読み取り可能なエラーコード（例：`"NETWORK_ERROR"`、`"TIMEOUT"`） |
