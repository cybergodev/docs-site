...
ドメインクライアントは内部でリクエストオプションを**2 回適用**します。1 回はセッション状態（Cookie、ヘッダー）のキャプチャ用、もう 1 回は実際のリクエスト用です。副作用のあるオプション（カウンター、nonce 生成など）は避けてください。このようなオプションが必要な場合は、基になる `Client` を使用してください。
:::
...
:::warning
`http://` と `https://` プレフィックスのパスのみが絶対 URL として認識されます。その他のプロトコル（例：`ftp://`）は絶対パスとして認識されず、相対パスとして結合されるため、通常はリクエストの失敗を招きます。
:::
...
- [インターフェース定義](../types/interfaces) - Client、Doer インターフェースリファレンス
