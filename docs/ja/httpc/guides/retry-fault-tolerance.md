...
| 条件 | リトライ |
|------|---------|
| ネットワークエラー（接続拒否、一時的/タイムアウト系 DNS 失敗など） | はい |
| タイムアウトエラー | はい |
...
// HTTPC は最大 60 秒待機してからリトライ（サーバー指定の 120s は安全上限の 60s に切り詰められます）

// サーバーが返す: Retry-After: Fri, 25 Apr 2026 12:00:00 GMT
// HTTPC は指定された時刻まで待機してからリトライ（現在時刻から 60s 超の場合は 60s に切り詰められます）
```
...
| 冪等な操作 | リトライ可能 |
| 非冪等な操作（POST） | ネットワークエラー時のみリトライを推奨（デフォルトは 5xx/408/429 もリトライするため、カスタム RetryPolicy で範囲を絞る必要があります） |
